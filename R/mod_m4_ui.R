      ...
      column(
        ...
        tabsetPanel(
          ...
          tabPanel(
            ...
            helpText("Error(T) + \\alpha \\cdot |T|  (\\alpha \\propto cp).  |T| es el número de hojas."),
            ...
            textAreaInput(ns("interpretacion_grafico"), "¿Dónde está el punto óptimo en el gráfico? ¿Por qué?",
                        rows = 3, placeholder = "El punto óptimo se ubica en X nodos porque a partir de ese tamaño el error no disminuye de manera significativa, por lo que un árbol más grande solo añade complejidad."),
          ...
          tabPanel(
            ...
            p("La matriz de confusión compara clases reales y predichas y resume aciertos y errores por categoría. En problemas multiclase, calculamos métricas por clase y promedios globales (macro y ponderado)."),
            ...
            p("Las métricas se calculan a partir de la matriz y se interpretan por clase y en promedio:"),
            tags$ul(
              tags$li(strong("Accuracy:"), " aciertos totales / total de casos."),
              tags$li(strong("Recall (sensibilidad) por clase:"), " TP / (TP + FN) para esa clase."),
              tags$li(strong("Precisión por clase:"), " TP / (TP + FP) para esa clase."),
              tags$li(strong("Especificidad por clase:"), " TN / (TN + FP) frente a esa clase."),
              tags$li(strong("F1 por clase:"), " 2 × (Precisión × Recall) / (Precisión + Recall)."),
              tags$li(strong("F1 macro:"), " promedio simple de F1 por clase; trata todas las clases por igual."),
              tags$li(strong("F1 ponderado:"), " promedio de F1 por clase ponderado por soporte; útil con desbalance.")
            ),
            p("El umbral de decisión aplica cuando comparas una clase objetivo frente al resto (p. ej., 'alto' vs no 'alto'). Al reducir el umbral, aumentas recall para 'alto' y aceptas más falsos positivos. Ajusta según la política de riesgo."),
            ...
            textAreaInput(ns("respuesta_metrica"), "Interpreta esta métrica en el contexto del modelo:",
                         rows = 3, placeholder = "Explica qué implica este valor para priorizar 'alto' riesgo. Indica si conviene ganar recall aunque aumenten falsos positivos en contextos de prevención."),
